...
void debug(){
	if(mode == TS_SPIN){
		...
		float tail_angle = joint[8].getPosition()+PI/2;
...
public:
	...
	void update() {
		...
		if(mode == TS_WAIT){
			joint[8].setOpenLoop(0); //don't do anything
			tLast = S->millis;
		}
		else if(mode == TS_SPIN){
			C->mode = RobotCommand_Mode_JOINT;

            // Following Section is to calculate for the torque the tail should provide
			// get body info
            ...
            // finished calculating torque for tail motor
		...
		// leg stuff
			...
			for (int i = 0; i < 4; ++i){
				...
				if (i == 0 || i == 1) {
					...
					limb[i].setGain(EXTENSION, 0.3, 0.04);
