...
enum FHMode
{
	FH_SIT = 0,
	FH_STAND,
	FH_START, FH_UP, FH_FORWARD, FH_FORWARD_2
};
...
public:
	...
	bool rightTurn = true;
	// bool ifRight[] = {false, false, true, true}; 
	...
	void update()
	{
		...
		else if (mode == FH_FORWARD)
		{
			...
			// right legs up
			limb[downLegs[0]].setGain(ANGLE, 0.6, 0.01);
			limb[downLegs[0]].setPosition(ANGLE, -S->imu.euler.y-0.1);
			limb[downLegs[0]].setGain(EXTENSION, 80, 4);
			// limb[downLegs[0]].setPosition(EXTENSION, 0.23);
			limb[downLegs[0]].setPosition(EXTENSION, 0.26);
			limb[downLegs[1]].setGain(ANGLE, 0.6, 0.01);
			limb[downLegs[1]].setPosition(ANGLE, -S->imu.euler.y+0.1);
			limb[downLegs[1]].setGain(EXTENSION, 80, 4);
			limb[downLegs[1]].setPosition(EXTENSION, 0.15);

			limb[upLegs[0]].setGain(ANGLE, 0.6, 0.01);
			limb[upLegs[0]].setPosition(ANGLE, -S->imu.euler.y+0.1);
			limb[upLegs[0]].setGain(EXTENSION, 50, 4);
			limb[upLegs[0]].setPosition(EXTENSION, 0.25);
			limb[upLegs[1]].setGain(ANGLE, 0.6, 0.01);
			limb[upLegs[1]].setPosition(ANGLE, -S->imu.euler.y+0.1);
			limb[upLegs[1]].setGain(EXTENSION, 50, 4);
			limb[upLegs[1]].setPosition(EXTENSION, 0.25);

			curMillis = S->millis;
			if(curMillis - oldMillis > 400)
			{
				mode = FH_FORWARD_2;
				oldMillis = S->millis;
			}
		...
		else if (mode == FH_FORWARD_2)
		{
			...
			limb[downLegs[0]].setGain(ANGLE, 0.6, 0.01);
			limb[downLegs[0]].setPosition(ANGLE, -S->imu.euler.y-0.1);
			limb[downLegs[0]].setGain(EXTENSION, 50, 4);
			// limb[downLegs[0]].setPosition(EXTENSION, 0.23);
			limb[downLegs[0]].setPosition(EXTENSION, 0.26);
			limb[downLegs[1]].setGain(ANGLE, 1.0, 0.01);
			limb[downLegs[1]].setPosition(ANGLE, -S->imu.euler.y+0.1);
			limb[downLegs[1]].setGain(EXTENSION, 100, 4);
			limb[downLegs[1]].setPosition(EXTENSION, 0.15);

			limb[upLegs[0]].setGain(ANGLE, 1.0, 0.01);
			limb[upLegs[0]].setPosition(ANGLE, -S->imu.euler.y-0.2);
			limb[upLegs[0]].setGain(EXTENSION, 100, 4);
			limb[upLegs[0]].setPosition(EXTENSION, 0.15);
			limb[upLegs[1]].setGain(ANGLE, 1.0, 0.01);
			limb[upLegs[1]].setPosition(ANGLE, -S->imu.euler.y-0.1);
			limb[upLegs[1]].setGain(EXTENSION, 100, 4);
			limb[upLegs[1]].setPosition(EXTENSION, 0.15);

			curMillis = S->millis;
			if(curMillis - oldMillis > 800)
			{
				rightTurn = !rightTurn; // switch side
				mode = FH_UP;
				oldMillis = S->millis;
			}
		}
