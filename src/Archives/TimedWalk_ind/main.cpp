...
enum FHMode
{
	...
	FH_BACKWARD, FH_BACKWARD_2
};

class IndWalk : public ReorientableBehavior
{
public:
	...
	void update()
	{
		...
		else if (mode == FH_FORWARD)
		{
			...
			limb[0].setPosition(EXTENSION, 0.14);
			// Leg 3
			...
			limb[3].setPosition(EXTENSION, 0.14);
			...
			limb[1].setPosition(EXTENSION, 0.14);
			// Leg 2
			...
			limb[2].setPosition(EXTENSION, 0.14);
...
void debug()
{
  ...
  // write(LOGGER_FILENO, (char *)&outs, sizeof(outs));
  ...
  // printf("%u %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n",
	...
	// if(limb[0].getExternalWrench(&extForce, &extTorque))
  	// printf("%u\t%.2f %.2f %.2f\n", S->millis, extForce.x, extForce.y, extForce.z);
}
